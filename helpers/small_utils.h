...
void print_dou(vector<double> vd)
{
    for(int i = 0; i < vd.size(); ++i){
        if( i != vd.size() - 1) { cout << vd[i] << ", "; }
        else { cout << vd[i] << ".\n"; }
    }
}

void print_str(vector<string> vs)
{
...
vector< vector<double> > read_dataset(string filepath)
{
    ...
    } else { throw "[read_dataset] Unable to open file."; }
}

void write_dataset_from_cin(string filepath, int lines)
{
    ...
    if(myfile.is_open())
    {
        ...
        {
            regex pat {R"(\s|[a-z]|[A-Z])"};

            cout << "Input the first vector:\n> ";
            string line;
            getline(cin, line);
            line = regex_replace(line, pat, "");
            cout << line;
        }
    }
}

void write_paths(string filepath, map<string, string> vs)
{
    ofstream myfile(filepath);
    if(myfile.is_open())
    {
...
map<string, string> read_paths(string filepath)
{
    ifstream myfile (filepath);
    if(myfile.is_open())
    {  
        ...
        throw "[read_paths] Can't open file.";
    }
